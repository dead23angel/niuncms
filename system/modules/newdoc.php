...
{
$result_index = Niun::getInstance()->Get('DataBase')->Query("SELECT `nameurl`, `title`, `id`, `cat` FROM blog WHERE viewindex!='0' AND pablick='1' ORDER BY date_b DESC LIMIT 5");
$myrow_index = Niun::getInstance()->Get('DataBase')->GetArray($result_index);

if($myrow_index != "")
{
$sm_read = Niun::getInstance()->Get('Template')->Fetch('newdoc');
